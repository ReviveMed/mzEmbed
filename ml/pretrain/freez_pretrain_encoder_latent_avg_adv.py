...
class MaskedCrossEntropyLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        # Use the predefined constant max loss value
        inverted_loss = 10 - loss  # Fixed max_loss_value to invert the loss
        
        #return -1*loss.mean()
        return inverted_loss.mean()  # Invert the loss and return the mean
...
def retrain_pretrain_adv_task(VAE_model,model_path, X_train, y_data_train, X_val, y_data_val, num_classes, num_layers_to_retrain=1, add_post_latent_layers=False, num_post_latent_layers=1, post_latent_layer_size=128, num_epochs=20, batch_size=32, learning_rate=1e-4, dropout=0.2, l1_reg_weight=0.0, l2_reg_weight=0.0, latent_passes=20, seed=None, patience=0):
    ...
    # Initialize LabelEncoder to encode class labels
    label_encoder = LabelEncoder()
    
    # Train Dataset
    y_data_train_filled = y_data_train.fillna(-1)
    mask_train = y_data_train_filled != -1  # Only true for values that are not -1
    y_data_train_encoded = y_data_train_filled.copy()
    y_data_train_encoded[mask_train] = label_encoder.fit_transform(y_data_train_filled[mask_train])
    y_data_train_encoded = y_data_train_encoded.astype(int)
    
    # Val dataset
    y_data_val_filled = y_data_val.fillna(-1)
    mask_val = y_data_val_filled != -1  # Only true for values that are not -1
    y_data_val_encoded = y_data_val_filled.copy()
    y_data_val_encoded[mask_val] = label_encoder.fit_transform(y_data_val_filled[mask_val])
    y_data_val_encoded = y_data_val_encoded.astype(int)


    # Convert pandas DataFrames to PyTorch tensors
    X_train_tensor = torch.tensor(X_train.values, dtype=torch.float32).to(device)
    y_data_train_tensor = torch.tensor(y_data_train_encoded.values, dtype=torch.long if num_classes > 2 else torch.float32).to(device)
    
    X_val_tensor = torch.tensor(X_val.values, dtype=torch.float32).to(device)
    y_data_val_tensor = torch.tensor(y_data_val_encoded.values, dtype=torch.long if num_classes > 2 else torch.float32).to(device)
