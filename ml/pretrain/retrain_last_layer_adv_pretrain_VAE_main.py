...
from pretrain.freez_pretrain_encoder_latent_avg_adv import retrain_pretrain_adv_task
...
def evalute_pretrain_latent_task_adv( best_model, X_data_train, X_data_val, X_data_test, y_data_train, y_data_val, y_data_test, task, num_classes, batch_size=32, latent_passes=20):
    ...
    def eval_dataset (model, X_data, y_data, num_classes, batch_size=32, latent_passes=20):
        ...
        #managing Y catengtorical data
        y_data_filled = y_data.fillna(-1)
        mask = y_data_filled != -1  # Only true for values that are not -1
        y_data_encoded = y_data_filled.copy()
        y_data_encoded[mask] = label_encoder.fit_transform(y_data_filled[mask])
        y_data_encoded = y_data_encoded.astype(int)
        
        y_tensor = torch.tensor(y_data_encoded.values, dtype=torch.long if num_classes > 2 else torch.float32).to(device)
...
def retrain_pretrain_VAE_adv_grid_search_optimization(
    pretrain_VAE_OG,
    ...
    for idx, param_values in enumerate(all_param_combinations):
        ...
        val_metrics = retrain_pretrain_adv_task(
    ...
    model_results_df = evalute_pretrain_latent_task_adv(
...
def main():
    ...
    results_combined_df= retrain_pretrain_VAE_adv_grid_search_optimization(pretrain_vae, X_data_train, y_data_train, X_data_val, y_data_val, X_data_test, y_data_test, task, seed, model_path, combined_params)
