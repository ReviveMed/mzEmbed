...
WEIGHTS_DCT = {
    'pretrain_recon': WEIGHT_PRETRAIN_RECON,
    'pretrain_clf': WEIGHT_PRETRAIN_CLF,
    'pretrain_adv': WEIGHT_PRETRAIN_ADV,
    'finetune_val': WEIGHT_FINETUNE_VAL,
    'finetune_traindiff': WEIGHT_FINETUNE_TRAINDIFF,
    'randinit_val': WEIGHT_RANDINIT_VAL
}
...
def objective(trial):
    ...
    pretrain_train_files = y_data[y_data['Pretrain']=='Train'].index.to_list()
    pretrain_val_files = y_data[y_data['Pretrain']=='Val'].index.to_list()
    ...
    num_folds = kwargs.get('num_folds', 1)
    hold_out_str_list = kwargs.get('hold_out_str_list', [])
    hold_out_iter = kwargs.get('hold_out_iter', 1)
    ...
    early_stopping_patience = pretrain_head_kwargs.get('early_stopping_patience', 0)
    ...
    pretrain_adv_col = y_pretrain_cols[1]
    pretrain_train_dataset = CompoundDataset(X_pretrain_train, y_pretrain_train[pretrain_head_col], y_pretrain_train[pretrain_adv_col])
    pretrain_val_dataset = CompoundDataset(X_pretrain_val, y_pretrain_val[pretrain_head_col], y_pretrain_val[pretrain_adv_col])

    if pretrain_head_kind != 'NA':
        num_classes_pretrain_head = pretrain_train_dataset.get_num_classes_head()
        weights_pretrain_head = pretrain_train_dataset.get_class_weights_head()
        pretrain_head.define_loss(class_weight=weights_pretrain_head)


    if pretrain_adv_kind != 'NA':      
        num_classes_pretrain_adv = pretrain_train_dataset.get_num_classes_adv()
        weights_pretrain_adv = pretrain_train_dataset.get_class_weights_adv()
        pretrain_adv.define_loss(class_weight=weights_pretrain_adv)

    if (pretrain_val_frac>0) and (early_stopping_patience>0):
        train_size = int((1-pretrain_val_frac) * len(pretrain_train_dataset))
        val_size = len(pretrain_train_dataset) - train_size

        pretrain_train_dataset, preval_dataset = torch.utils.data.random_split(pretrain_train_dataset, [train_size, val_size])
        val_loader = torch.utils.data.DataLoader(preval_dataset, batch_size=pretrain_batch_size, shuffle=False)

    dataloaders = {
        'train': torch.utils.data.DataLoader(pretrain_train_dataset, batch_size=pretrain_batch_size, shuffle=True),
    }
    if (pretrain_val_frac>0) and (early_stopping_patience>0):
    ...
    obj_pretrain_clf = output['end_state_eval']['test']['head_auc']
    ...
    if 'test LogisticRegression_auc' in output['sklearn_adversary_eval']:
        ...
        obj_pretrain_adv = output['sklearn_adversary_eval']['test LogisticRegression_auc']
    else:
        obj_pretrain_adv = 0
    ...
    if pretrain_head_kind != 'NA':
        num_classes_pretrain_head = pretrain_trainval_dataset.get_num_classes_head()
        weights_pretrain_head = pretrain_trainval_dataset.get_class_weights_head()
        pretrain_head.define_loss(class_weight=weights_pretrain_head)

    if pretrain_adv_kind != 'NA':
        num_classes_pretrain_adv = pretrain_trainval_dataset.get_num_classes_adv()
        weights_pretrain_adv = pretrain_trainval_dataset.get_class_weights_adv()
        pretrain_adv.define_loss(class_weight=weights_pretrain_adv)

    if (pretrain_val_frac>0) and (early_stopping_patience>0):
        train_size = int((1-pretrain_val_frac) * len(pretrain_trainval_dataset))
        val_size = len(pretrain_trainval_dataset) - train_size

        pretrain_trainval_dataset, preval_dataset = torch.utils.data.random_split(pretrain_trainval_dataset, [train_size, val_size])
        val_loader = torch.utils.data.DataLoader(preval_dataset, batch_size=pretrain_batch_size, shuffle=False)

    dataloaders = {
        'train': torch.utils.data.DataLoader(pretrain_trainval_dataset, batch_size=pretrain_batch_size, shuffle=True),
    }

    if (pretrain_val_frac>0) and (early_stopping_patience>0):
        dataloaders['val'] = val_loader

    dataloaders['test'] = torch.utils.data.DataLoader(pretrain_test_dataset, batch_size=pretrain_batch_size, shuffle=False)
    ...
    trial.set_user_attr('Pretrain Recon Train+Val loss', output['end_state_losses']['train']['reconstruction'])
    trial.set_user_attr('Pretrain Recon Test loss', output['end_state_losses']['test']['reconstruction'])
    
    trial.set_user_attr('Pretrain Head Train+Val AUC', output['end_state_eval']['train']['head_auc'])
    ...
    for n_fold in range(num_folds):
        ...
        train_dataset = CompoundDataset(X_train, y_train[finetune_head_col], y_train[finetune_adv_col])
        test_dataset = CompoundDataset(X_test, y_test[finetune_head_col], y_test[finetune_adv_col])
        ...
        # if NUM_OBJECTIVES==1:
        #     trial.report(res['Val AUC'], n_fold)
        #     if trial.should_prune(n_fold):
        #         raise optuna.TrialPruned()

        if res['Val AUC'] < MIN_ACCEPTABLE_AUC*0.8:
            print('intermediate finetune Val AUC is much lower than the minimum acceptable AUC. Pruning')
            raise optuna.TrialPruned()
    ...
    obj_finetune_traindiff = obj_finetune_train - obj_finetune_val
    # obj_finetune_robust = obj_finetune_val / (obj_finetune_train / obj_finetune_val)

    # prune bad trials
    if obj_finetune_val < MIN_ACCEPTABLE_AUC:
        print('finetune Val AUC is lower than the minimum acceptable AUC. Pruning')
        raise optuna.TrialPruned()

    if obj_finetune_train < 0.99*obj_finetune_val:
        print('finetune Train AUC is lower than the Val AUC. Pruning')
        raise optuna.TrialPruned()
    ...
    if len(hold_out_str_list)> 0:
        ...
        results = []
        for iter in range(hold_out_iter):
            res = {}
    ...
    generalized_obj = 0 + \
        (-1)*WEIGHT_PRETRAIN_RECON*obj_pretrain_recon + \
        WEIGHT_PRETRAIN_CLF*obj_pretrain_clf + \
        (-1)*WEIGHT_PRETRAIN_ADV*obj_pretrain_adv + \
        WEIGHT_FINETUNE_VAL*obj_finetune_val + \
        (-1)*WEIGHT_FINETUNE_TRAINDIFF*(obj_finetune_traindiff) + \
        (-1)*WEIGHT_RANDINIT_VAL*obj_randinit_val

    result_save_file = os.path.join(save_dir, 'result.json')
    save_json(result_dct, result_save_file)

    return generalized_obj
