...
def get_kwargs(trial,study_kind,goal_col=None,result_dir=None,study_name=None):

    if study_name is None:
        if goal_col is None:
            study_name = study_kind
        else:
    ...
    elif 'march20' in study_kind:
        kwargs= _get_study_march20(trial,goal_col,result_dir=result_dir)    
        kwargs = _get_adv_kwargs(kwargs,trial)
        kwargs = _get_clf_kwargs(kwargs,trial)
        return kwargs

    if 'finetune_S1' in study_kind:
        #finetune the best model from pretain_1_S
        kwargs = _get_study_finetune1(trial,goal_col,result_dir=result_dir)    
...
def _get_study_march20(trial, goal_col, result_dir):
    ...
    use_finetune_weight_decay = False
    
    use_pretrain_elasticnet_reg = trial.suggest_categorical('use_pretrain_elasticnet_reg', [True, False])
    #use_finetune_elasticnet_reg = trial.suggest_categorical('use_finetune_elasticnet_reg', [True, False])
    use_finetune_elasticnet_reg = False
    ...
    kwargs = {
        ...
        'pretrain_kwargs': {
            ...
            'adversarial_mini_epochs': trial.suggest_int('pretrain_adv_mini_epochs', 1, 10, step=1),
        },
        ...
        'finetune_kwargs': {
            ...
            'num_epochs': 1, # trial.suggest_int('finetune_epochs', 10, 250,log=True),
            'lr': 0.01, # trial.suggest_float('finetune_lr', 0.0001, 0.01, log=True),
            ...
            'head_weight': 1,
            'adversary_weight': 0,
            'noise_factor': 0, #trial.suggest_float('finetune_noise_factor', 0, 0.25, step=0.05),
...
def _get_study_finetune1(trial, goal_col, result_dir):
    ...
    use_pretrain_weight_decay = False #trial.suggest_categorical('use_pretrain_weight_decay', [True, False])
    use_finetune_weight_decay = trial.suggest_categorical('use_finetune_weight_decay', [True, False])
    use_pretrain_elasticnet_reg = False #trial.suggest_categorical('use_pretrain_elasticnet_reg', [True, False])
    use_finetune_elasticnet_reg = trial.suggest_categorical('use_finetune_elasticnet_reg', [True, False])
    if use_pretrain_weight_decay:
        pretrain_weight_decay = trial.suggest_float('pretrain_weight_decay', 0.0001, 0.1, log=True)
    else:
    ...
    kwargs = {
        ...
        'pretrain_kwargs': {
            # 'num_epochs': trial.suggest_int('pretrain_epochs', 10, 100,log=True),
            'num_epochs': 75, #trial.suggest_int('pretrain_epochs', 10, 500,log=True),
            'lr': 0.0003, #trial.suggest_float('pretrain_lr', 0.0001, 0.01, log=True),
            'weight_decay': pretrain_weight_decay,
            'l1_reg_weight': pretrain_l1_reg_weight,
            'l2_reg_weight': pretrain_l2_reg_weight,
            'encoder_weight': 1,
            'head_weight': 0.01086,
            'adversary_weight': 0.05094,
            'noise_factor': 0, #trial.suggest_float('pretrain_noise_factor', 0, 0.25, step=0.05),
            'early_stopping_patience': 0, #trial.suggest_int('pretrain_early_stopping_patience', 0, 50, step=10),
            'loss_avg_beta': -1,
            # 'loss_avg_beta': 0,
            # 'end_state_eval_funcs': {},
            'end_state_eval_funcs': get_end_state_eval_funcs(),
