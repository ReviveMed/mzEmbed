...
def compute_losses(model, X_data_train, X_data_val, X_data_test, device):
    ...
    def compute_recon_kl_loss(x, model):
        with torch.no_grad():  # Disable gradient computation
            ...
            recon_x, mu, log_var, supervised_out = model(x)  # Forward pass
...
def main():
    ...
    parser.add_argument('--task_name', type=str, default='',
                        help='the name of the task that models was trained on.')
    ...
    for index, row in pretrain_model_df.iterrows():
        ...
        task_for_filename = task_name.replace(' ', '_')
        
        ## TL True 
        finetune_VAE_TL_file= f'{models_path}/{task_for_filename}_finetune_VAE_TL_True_best_model_state.pt'
        finetune_VAE_TL=torch.load(finetune_VAE_TL_file)

         ## TL False 
        finetune_VAE_noTL_file= f'{models_path}/{task_for_filename}_finetune_VAE_TL_False_best_model_state.pt'
    ...
    results_df.to_csv(f'{finetune_save_dir}/{pretrain_name}/{task_for_filename}_fine_tune_TL_noTL_{pretrain_name}_latent_eval_results.csv', index=False)

    print (f'Predictions are saved in: {finetune_save_dir}/{pretrain_name}/{task_for_filename}_fine_tune_TL_noTL_{pretrain_name}_latent_eval_results.csv')
