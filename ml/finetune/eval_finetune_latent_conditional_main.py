...
def generate_latent_space(X_data, model, batch_size=128):
...
def compute_losses(model, X_data_train, X_data_val, X_data_test, device):
    ...
    def compute_recon_kl_loss(x, model):
        with torch.no_grad():  # Disable gradient computation
            ...
            recon_x, mu, log_var = model.forward_x(x)  # Forward pass
...
def main():
    ...
    parser.add_argument('--condition_list', type=str, default='OS,OS_event',
                        help='the list of conditions that was used for the conditional VAE model, sperated by comma.')
    ...
    for index, row in pretrain_model_df.iterrows():
        ...
        condition_list_for_filename = condition_list.replace(' ', '_')
    ...
    results_df.to_csv(f'{finetune_save_dir}/{pretrain_name}/{condition_list_for_filename}_fine_tune_TL_noTL_{pretrain_name}_latent_eval_results.csv', index=False)

    print (f'Predictions are saved in: {finetune_save_dir}/{pretrain_name}/{condition_list_for_filename}_fine_tune_TL_noTL_{pretrain_name}_latent_eval_results.csv')
