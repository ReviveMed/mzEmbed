...
class CompoundDataset(Dataset):
    def __init__(self, X, y_head, y_adv):
    ...
    def __getitem__(self, idx):
        return self.X[idx], self.y_head[idx], self.y_adv[idx]
                    ...
                    if phase == 'train':
                        ...
                        if encoder_weight > 0:
                            encoder_loss, encoder_loss_avg = normalize_loss(
                                encoder_loss, encoder_loss_avg, beta, curr_batch)

                        if head_weight > 0:
                            head_loss, head_loss_avg = normalize_loss(
                                head_loss, head_loss_avg, beta, curr_batch)

                        if adversary_weight > 0:
                            adversary_loss, adversary_loss_avg = normalize_loss(
                                adversary_loss, adversary_loss_avg, beta, curr_batch)
                        ...
                        if encoder_weight > 0:
                            encoder_loss, _ = normalize_loss(
                                encoder_loss, encoder_loss_avg, beta, -1)
                        
                        if head_weight > 0:
                            head_loss, _ = normalize_loss(
                                head_loss, head_loss_avg, beta, -1)
                        
                        if adversary_weight > 0:
                            adversary_loss, _ = normalize_loss(
                                adversary_loss, adversary_loss_avg, beta, -1)
...
def evaluate_head_accuracy(head_ouputs, head_targets, adversary_outputs, adversary_targets, head, adversary):
    ...
    nan_mask = ~np.isnan(head_targets)
    head_acc = balanced_accuracy_score(head_targets[nan_mask], head_preds[nan_mask])
    return head_acc
...
def evaluate_adversary_accuracy(head_ouputs, head_targets, adversary_outputs, adversary_targets,head, adversary):
    ...
    nan_mask = ~np.isnan(adversary_targets)
    adversary_acc = balanced_accuracy_score(adversary_targets[nan_mask], adversary_preds[nan_mask])
...
def evaluate_head_auc(head_ouputs, head_targets, adversary_outputs, adversary_targets, head, adversary):
    ...
    if head.num_classes > 2:
        head_auc = roc_auc_score(head_targets[nan_mask], head_probs[nan_mask],
...
def evaluate_adversary_auc(head_ouputs, head_targets, adversary_outputs, adversary_targets,head, adversary):
    ...
    nan_mask = ~np.isnan(adversary_targets)
    if adversary.num_classes > 2:
        adversary_auc = roc_auc_score(adversary_targets[nan_mask], adversary_probs[nan_mask],
                                    average='weighted', multi_class='ovo')
    else:
        adversary_auc = roc_auc_score(adversary_targets[nan_mask], adversary_probs[nan_mask],
                                    average='weighted')
...
if __name__ == "__main__":
    ...
    # model_id = 'feb26_AE_with_adversary-5_and_head-0.2'
    model_id = 'feb26_AE_4_with_head-0.2_and_adversary-5'
    ...
    for subset_num in [0]:
        ...
        if num_classes_head > 2:
            head = MultiClassClassifier(latent_dim, hidden_size=4, num_hidden_layers=1, num_classes=num_classes_head)
        else:
            head = BinaryClassifier(latent_dim, hidden_size=4, num_hidden_layers=1)
