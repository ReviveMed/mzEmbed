...
def classifier_training_wrapper(data_dir,**kwargs):
    ...
    for subset_id in range(n_subsets):
        ...
        initial_train_dataset = ClassifierDataset(data_dir, 
                                    subset='train_{}'.format(subset_id),
                                    label_col=label_col,
                                    label_encoder=label_encoder)
        
        class_weights = 1 / torch.bincount(initial_train_dataset.y.long())
        ...
        if (val_fraction > 0) and (early_stopping > 0):
            ...
            # train_dataset = initial_train_dataset[train_dataset.indices]
            # val_dataset = initial_train_dataset[val_dataset.indices]
        else:
...
def run_classification_training(dataloaders,**kwargs):
    ...
    task = kwargs.get('task', 'multiclass') #for torchmetrics
    ...
    running_accuracy = Accuracy(task=task,average=how_average,num_classes=num_classes).to(device)
    running_auroc = AUROC(task=task,average=how_average,num_classes=num_classes).to(device)
