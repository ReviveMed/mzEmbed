...
def create_sklearn_model_summary(base_model,model_name,param_grid,input_dir):
    ...
    for ii in range(len(rs.cv_results_['params'])):
        results_list.append({'val_scores': [], 'test_scores': [], 'train_scores': []})
    ...
    if use_predefined_val:
        ...
        for i in range(n_repeats):
            ...
            for ii,model in enumerate(model_list):
                    ...
                    results['train_scores'].append(roc_auc_score(y_train, train_preds))
...
def run_performance_eval(n_layers,n_neurons,activation,
    ...
    if epoch_list is None:
        ...
        epoch_list = [50,75,100,125,150,200,250,300,350,400]
    ...
    if custom_encoder_path is not None:
        ...
        for encoder_patience in [10]:#,15,20]:
...
# input_dir = '/Users/jonaheaton/Desktop/cohort_combine_oct16/selected_studiesX 13/combat_survival_class_task'
...
if True:
    ...
    for n_layers in [0]:
        for n_neurons in [32,64,96,128]:
            run_performance_eval(n_layers,n_neurons,'sigmoid',
                                input_dir=input_dir,
                                output_subdir='keras_autoencoder_models4')
