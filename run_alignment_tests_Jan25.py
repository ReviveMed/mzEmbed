...
def initialize_study_comparison(targets_path0, targets_path1, peaks_obj_path0, peaks_obj_path1, name0, name1, output_dir, **kwargs):
    ...
    if os.path.exists(target_peak_matches1_path):
        ...
        matches_0 = peaks_to_targets_wrapper(peak_info, targets_common_0, rt_tol=rt_tol_matching, mz_tol=mz_tol)
    ...
    if os.path.exists(target_peak_matches1_path):
        ...
        matches_1 = peaks_to_targets_wrapper(peak_info, targets_common_1, rt_tol=rt_tol_matching, mz_tol=mz_tol)
...
def align_study_comparison(input_info,run_params,yes_plot=True,n_neighbors=20,alignment_method='Eclipse'):
    ...
    alignment_result = align_ms_studies(peaks0, peaks1, name0, name1, alignment_method, alignment_params, verbose=True)
        ...
        for i1, result_id1 in enumerate(result_id_list):
            ...
            for param_file in run_param_files:
                ...
                if 'alignment_method' not in run_params:
                    ...
                    if alignment_method not in ['Eclipse','metabCombiner','Merge']:
...
if __name__ == '__main__':
    ...
    comparison_id_subset = None
    comparison_id_subset_file = '/Users/jonaheaton/Desktop/alignment_analysis/Output_Data/comparison_id_list subset.txt'
    if comparison_id_subset_file is not None:
        with open(comparison_id_subset_file, 'r') as f:
            comparison_id_subset = f.read().splitlines()
        comparison_id_subset = [x for x in comparison_id_subset if x != '']
        comparison_id_subset = [x for x in comparison_id_subset if x[0] != '#']
        print(f'comparison_id_subset: {comparison_id_subset}')
    ...
    if method_metabC_grid_search:
        ...
        run_param_file_names = ['Merge_50_50_Jan25.json','Eclipse_50_50.json','metabCombiner_50_50_Jan23.json']
    ...
    if skip_alignment:
        ...
        for mode in mode_list:
            comparison_id_list = main(mode,
                                    ...
                                    comparison_id_subset=comparison_id_subset)
